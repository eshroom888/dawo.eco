"""YouTube Validator - checks EU compliance for research items.
...
Usage:
    validator = YouTubeValidator(compliance_checker)
...
class YouTubeValidator:
    """YouTube Validator - EU compliance checking for research items.
    ...
    async def validate(
        ...
        logger.info(f"Validating {len(items)} items for EU compliance")
        ...
        for item in items:
            try:
                ...
                logger.error(
                    f"Failed to validate item '{item.title[:50]}': {e}"
                )
                # Skip item on validation failure
                continue

        logger.info(
            f"Validation complete: {stats['compliant']} compliant, "
            f"{stats['warning']} warnings, {stats['rejected']} rejected"
        )
    ...
    async def _validate_single(
        ...
        logger.debug(
            f"Validated '{item.title[:30]}': {compliance_status}"
        )
